Shader "Hidden/SC Post Effects/Mosaic"
{
		...
		float2 HexUV(float2 hexIndex) {
			...
			float2 r;
			r.x = i * _Params.x;
			r.y = j * _Params.y + (i % 2) * _Params.y / 2.;
			return r;
		}
		...
		float2 HexIndex(float2 uv, float size) {
			...
			float yts = uv.y - float(it % 2) * _Params.y / 2.;
			int jt = int(floor((1. / _Params.y) * yts));
			float xt = uv.x - it * size;
			float yt = yts - jt * _Params.y;
			int deltaj = (yt > _Params.y / 2.) ? 1 : 0;
			float fcond = size * (2. / 3.) * abs(0.5 - yt / _Params.y);
