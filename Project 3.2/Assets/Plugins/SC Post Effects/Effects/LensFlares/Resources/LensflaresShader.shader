Shader "Hidden/SC Post Effects/Lensflares"
{
		...
		float4 FragGhosting(VaryingsDefault i) : SV_Target
	{
		...
		//Radial mask
		float falloff = length(float2(centerVec - frac(texcoord)) / 0.70);
		falloff = pow(1 - falloff, _Falloff);
		...
		float4 FragDebug(VaryingsDefault i) : SV_Target
	{
		...
		flares = ChromaticAberration(_FlaresTex, sampler_FlaresTex, _FlaresTex_TexelSize, i.texcoord, _ChromaticAbberation);
		flares *= mask;
