Shader "Hidden/SC Post Effects/Ripples"
{
	...
	float4 Frag(VaryingsDefault i): SV_Target
	{
		...
		float r = - (x * x + y * y);
		float z = 1 + 0.5 * sin((r + (_Time.x * _Speed)) / _Distance);

		float ripples = z * center;

		float4 screenColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv + (ripples * _Strength));
	...
	//w: Intensity
	float4 FragPerlin(VaryingsDefault i) : SV_Target
	{
		float2 uv = i.texcoordStereo;

		float3 noiseCoords = float3(uv.x * (_Distance * 100), uv.y * (_Distance * 100), (_Time.y * _Speed/4));
		half noise = 0.5 + 0.5 * snoise(noiseCoords);

		float ripples = noise;

		//return float4(ripples.r, ripples.r, ripples.r, 1);
