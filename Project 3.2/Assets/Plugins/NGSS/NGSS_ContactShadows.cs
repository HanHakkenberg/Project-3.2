...
public class NGSS_ContactShadows : MonoBehaviour
{
    ...
    void AddCommandBuffers()
    {
        ...
        if (mCamera)
        {
            foreach (CommandBuffer cb in mCamera.GetCommandBuffers(forward? UnityEngine.Rendering.CameraEvent.AfterDepthTexture : CameraEvent.BeforeLighting)) { if (cb.name == computeShadowsCB.name) { return; } }
